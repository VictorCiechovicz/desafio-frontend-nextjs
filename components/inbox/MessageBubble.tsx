...
function MessageBubbleImpl({ message, onRetry }: Props) {
  ...
  return (
    <div className={clsx("flex w-full", isOut ? "justify-end" : "justify-start")}>
      <div
        className={clsx(
          "relative max-w-[78%] rounded-lg px-3 py-2 text-sm shadow-sm md:max-w-[65%]",
          isOut
            ? "rounded-tr-sm bg-bubble-out text-bubble-out-foreground"
            : "rounded-tl-sm border border-border bg-card text-card-foreground",
          isPending && "opacity-70",
          isError && "border border-red-400/80 dark:border-red-500/80",
        )}
      >
          ...
          <div className="mt-1 flex items-center gap-1.5 text-[11px] text-red-600 dark:text-red-400">
            ...
            {onRetry && (
              <button
                ...
                className="font-semibold underline underline-offset-2 hover:text-red-700 dark:hover:text-red-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400/40"
              >
                Tentar novamente?
              </button>
            )}
